/*
** $Id: cursor-lite.c,v 1.51 2004/04/16 08:39:36 weiym Exp $
**
...
#ifdef _USE_NEWGAL
    if (colornum == 1) {
        ExpandMonoBitmap (HDC_SCREEN, pcsr->AndBits, __mg_csrimgpitch, pANDBits, MONOPITCH,
                        w, h, MYBMP_FLOW_UP, 0, 0xFFFFFFFF);
        ExpandMonoBitmap (HDC_SCREEN, pcsr->XorBits, __mg_csrimgpitch, pXORBits, MONOPITCH,
                        w, h, MYBMP_FLOW_UP, 0, 0xFFFFFFFF);
    }
    else if (colornum == 4) {
        ExpandMonoBitmap (HDC_SCREEN, pcsr->AndBits, __mg_csrimgpitch, pANDBits, MONOPITCH,
                        w, h, MYBMP_FLOW_UP, 0, 0xFFFFFFFF);
        Expand16CBitmap (HDC_SCREEN, pcsr->XorBits, __mg_csrimgpitch, pXORBits, MONOPITCH*4,
                        w, h, MYBMP_FLOW_UP, NULL);
    }
#else
    if (colornum == 1) {
        ExpandMonoBitmap (HDC_SCREEN, w, h, pANDBits, MONOPITCH, MYBMP_FLOW_UP,
                         pcsr->AndBits, __mg_csrimgpitch, 0, 0xFFFFFFFF);
        ExpandMonoBitmap (HDC_SCREEN, w, h, pXORBits, MONOPITCH, MYBMP_FLOW_UP,
                         pcsr->XorBits, __mg_csrimgpitch, 0, 0xFFFFFFFF);
    }
    else if (colornum == 4) {
        ExpandMonoBitmap (HDC_SCREEN, w, h, pANDBits, MONOPITCH, MYBMP_FLOW_UP,
                         pcsr->AndBits, __mg_csrimgpitch, 0, 0xFFFFFFFF);
        Expand16CBitmap (HDC_SCREEN, w, h, pXORBits, MONOPITCH*4, MYBMP_FLOW_UP,
                        pcsr->XorBits, __mg_csrimgpitch, NULL);
    }
#endif /* _USE_NEWGAL */
                     ...
                     const BYTE* pANDBits, const BYTE* pXORBits, int colornum)
{
    if (mgIsServer) {
        ...
        if (cli_request (&req, &hcursor, sizeof (HCURSOR)) < 0)
            hcursor = 0;
...
BOOL GUIAPI DestroyCursor (HCURSOR hcsr)
{
    if (mgIsServer) {
        ...
        if (cli_request (&req, &ret_value, sizeof (BOOL)) < 0)
            return FALSE;
...
static void hidecursor (void)
{
    ...
    GAL_PutBox (PHYSICALGC, CSR_OLDBOXLEFT, CSR_OLDBOXTOP, 
                    CURSORWIDTH, CURSORHEIGHT, CSR_SAVEDBITS);
}
...
static void showcursor (void)
{
    ...
    GAL_DisableClipping (PHYSICALGC);
    GAL_GetBox (PHYSICALGC, x, y, CURSORWIDTH, CURSORHEIGHT, CSR_SAVEDBITS);
    CSR_OLDBOXLEFT = x;
    CSR_OLDBOXTOP = y;

    memcpy (cursorbits, CSR_SAVEDBITS, __mg_csrimgsize);
    {
        int i;
        Uint32* dst = (Uint32*) cursorbits;
        Uint32* andbits = (Uint32*) CSR_CURRENT->AndBits;
...
HCURSOR GUIAPI SetCursorEx (HCURSOR hcsr, BOOL setdef)
{
    ...
    if (!mgIsServer) {
        ...
        if (cli_request (&req, &old, sizeof (PCURSOR)) < 0)
            return 0;
...
HCURSOR GUIAPI GetCurrentCursor (void)
{
    ...
    if (!mgIsServer) {
        ...
        if (cli_request (&req, &hcsr, sizeof (HCURSOR)) < 0)
            return 0;
...
#ifndef _USE_NEWGAL
        /* avoiding dead lock. */
        if (fShow && (get_cursor_sem_pid () == getpid ())) {
            unlock_cursor_sem ();
        }
#endif
        return;
...
void GUIAPI GetClipCursor (RECT* prc)
{
    if (!mgIsServer) {
        ...
        cli_request (&req, prc, sizeof (RECT));
        return;
    }
