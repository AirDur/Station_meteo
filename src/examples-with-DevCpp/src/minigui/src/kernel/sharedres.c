/*
** $Id: sharedres.c,v 1.48 2004/06/22 10:21:18 yaoyy Exp $
** 
...
static BOOL LoadCursorRes (void)
{
    ...
    __mg_csrimgpitch = CURSORWIDTH * BYTESPERPHYPIXEL;
    __mg_csrimgsize = __mg_csrimgpitch * CURSORHEIGHT;
#endif
...
static BOOL LoadBitmapRes (void)
{
    ...
    for (i = 0; i < nBmpNr; i++) {
        ...
        temp += sizeof (BITMAP);
        if (size) memcpy (temp, bmp.bmBits, size);
