...
{
    static PMAINWIN __mgs_captured_win;
    ...
    switch (message) {
        ...
        case MSG_RBUTTONDOWN:
            if (pUnderPointer) {
                ...
                SendNotifyMessage (pWin->hActiveChild, MSG_MOUSEACTIVE, HT_OUT, 0);
        ...
        case MSG_RBUTTONUP:
            if (__mgs_captured_ctrl) {
                PostMessage ((HWND)__mgs_captured_ctrl, message + MSG_NCMOUSEOFF,
                        CapHitCode, MAKELONG (x, y));
                ReleaseCapture ();
                __mgs_captured_win = NULL;
                __mgs_captured_ctrl = NULL;
        ...
        case MSG_MOUSEACTIVE:
            if (pWin->WinType == TYPE_CONTROL 
                ...
                if (wParam != HT_OUT) {
                    ...
                    PCONTROL old_active;
                    pCtrl = (PCONTROL)pWin;
                    old_active = pCtrl->pParent->active;
...
static void wndDrawCaption(const MAINWIN* pWin, HDC hdc, BOOL bFocus)
{
    ...
    TextOut(hdc, iBorder + iIconX + 2, iBorder + 3,pWin->spCaption);
...
// this function is CONTROL safe.
void wndDrawNCFrame(MAINWIN* pWin, HDC hdc, const RECT* prcInvalid)
{
    BOOL fGetDC;
    fGetDC = FALSE;
...
// this function is CONTROL safe.
void wndActiveMainWindow (PMAINWIN pWin, BOOL fActive)
{
    HDC hdc;
...
static int vs_get_absstart (PMAINWIN pWin, const RECT* rcVBar)
{
    ...
    limit = rcVBar->bottom - GetMainWinMetrics (MWM_CYVSCROLL);
...
static int hs_get_absstart (PMAINWIN pWin, const RECT* rcHBar)
{
    ...
    limit = rcHBar->right - GetMainWinMetrics (MWM_CXHSCROLL);
...
HWND GUIAPI CreateMainWindow(PMAINWINCREATE pCreateInfo)
{
    ...
    if( !(pWin = malloc(sizeof(MAINWIN))) ) 
       return HWND_INVALID;
    memset(pWin,0,sizeof(MAINWIN));
...
BOOL GUIAPI DestroyMainWindow (HWND hWnd)
{
    ...
    if (sg_repeat_msg.hwnd == hWnd)
        sg_repeat_msg.hwnd = HWND_DESKTOP;
...
BOOL GUIAPI InvalidateRect (HWND hWnd, const RECT* prc, BOOL bEraseBkgnd)
{
    ...
    if (pCtrl && pCtrl->WinType == TYPE_CONTROL && pCtrl->dwExStyle & WS_EX_TRANSPARENT) {
        RECT rcInParent;
                  ...
                  HWND hParentWnd, DWORD dwAddData)
{
    ...
    memset(pNewCtrl,0,sizeof (CONTROL));
...
/*
        if (set_window_element_data (hwnd, WET_METRICS, item, new_data, &old_data) >= 0)
            return (int)old_data;
*/
    }
...
/*
        if (set_window_element_data (hwnd, WET_COLOR, item, new_data, &old_data) >= 0)
