/*
** $Id: gif.c,v 1.10 2004/12/28 10:03:30 weiym Exp $
** 
...
int __mg_load_gif (MG_RWops *src, MYBITMAP* bmp, RGB* pal)
{
...
#ifdef _DEBUG
        uHALr_printf( "__mg_load_gif: GIF version number is not 87a or 89a\n");
#endif
    ...
    if (!ReadOK (src, buf, 7)) {
#ifdef _DEBUG
        uHALr_printf( "__mg_load_gif: bad screen descriptor\n");
#endif
    ...
    if (BitSet(buf[4], LOCALCOLORMAP)) {        /* Global Colormap */
        if (ReadColorMap(src, GifScreen.BitPixel, GifScreen.ColorMap,
                         &GifScreen.GrayScale)) {
#ifdef _DEBUG
            uHALr_printf( "__mg_load_gif: bad global colormap\n");
#endif
            return ERR_BMP_LOAD;                /* image loading error*/
        }
    }
    ...
    do {
        if (!ReadOK (src, &c, 1)) {
#ifdef _DEBUG
            uHALr_printf( "__mg_load_gif: EOF on image data\n");
#endif
            goto done;
        }
        if (c == ';') {                /* GIF terminator */
...
#ifdef _DEBUG
                uHALr_printf( "__mg_load_gif: no image %d of %d\n", imageNumber,imageCount);
#endif
                goto done;
            }
        }
        if (c == '!') {                /* Extension */
            if (!ReadOK (src, &c, 1)) {
#ifdef _DEBUG
                uHALr_printf( "__mg_load_gif: EOF on extension function code\n");
#endif
                goto done;
            }
            DoExtension (src, c);
        ...
        if (!ReadOK (src, buf, 9)) {
#ifdef _DEBUG
            uHALr_printf( "__mg_load_gif: bad image size\n");
#endif
            goto done;
        }
...
#ifdef _DEBUG
                uHALr_printf( "__mg_load_gif: bad local colormap\n");
#endif
                goto done;
            }
...
GetCode(MG_RWops *src, int code_size, int flag)
{
    ...
    if ((curbit + code_size) >= lastbit) {
        if (done) {
            if (curbit >= lastbit)
#ifdef _DEBUG
                uHALr_printf("__mg_load_gif: bad decode\n");
#endif
            return -1;
        }
...
LWZReadByte(MG_RWops *src, int flag, int input_code_size)
{
    ...
    while ((code = GetCode(src, code_size, FALSE)) >= 0) {
        if (code == clear_code) {
            ...
            if (count != 0) {
                /*
                 * uHALr_printf("missing EOD in data stream (common occurence)");
                 */
            }
            return -2;
        }
        ...
        while (code >= clear_code) {
            *sp++ = table[1][code];
            if (code == table[0][code]) {
                uHALr_printf( "__mg_load_gif: circular table entry\n");
                return -1;
            }
            code = table[0][code];
        }
          ...
          int gray, int interlace, int ignore)
{
    ...
    if (LWZReadByte(src, TRUE, c) < 0) {
        uHALr_printf("__mg_load_gif: error reading image\n");
        return 0;
    }
