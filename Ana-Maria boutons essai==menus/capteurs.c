...
int configuration_i2c(int buffer, unsigned char mask) {
  ...
  if (code_erreur < 0) { //si le fichier n'existe pas on affiche un message d'erreur
...
int lecture_temperature_Celsius(int buffer, double * Temperature) {
  ...
  donnees_brut = (bus[0] << 4) | (bus[1] >> 4); * Temperature = calcul_temperature_Celsius(donnees_brut); // 0.0625 = 128 / 2048
...
int lecture_temperature_Farenheit(int buffer, double * Temperature) {
  ...
  donnees_brut = (bus[0] << 4) | (bus[1] >> 4); * Temperature = calcul_temperature_Farenheit(donnees_brut); // 0.0625 = 128 / 2048
 ...
 ***********          ***********/
 ...
 int ADC_to_humidity0_poucentage(double temperature,double pression, double *humidite, int buffer) {
   ...
   // Calcul humidite théorique

   *humidite = humidite_pression_temperature0(pression,temperature);
