/*
** $Id: simedit.c,v 1.27 2005/02/01 09:53:42 clear Exp $
**
...
{   
    ...
    {
        case MSG_CREATE:
            if (!(simed = malloc (sizeof (SIMEDITDATA)))) {
                return -1;
            }
            memset(simed,0,sizeof (SIMEDITDATA));
            
            simed->status         = 0;
            simed->bufferLen      = LEN_SIMEDIT_BUFFER;
            simed->buffer         = malloc (LEN_SIMEDIT_BUFFER * sizeof (char));

            if (simed->buffer == NULL) {
                free (simed);
                return -1;
            }
            memset(simed->buffer,0,LEN_SIMEDIT_BUFFER * sizeof (char));
